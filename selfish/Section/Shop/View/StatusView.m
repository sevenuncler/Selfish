...
- (void)layoutSubviews {
    ...
    CGSize size = [self.contentLabel sizeThatFits:CGSizeMake(self.size.width - 3*PADDING, CGFLOAT_MAX)];
    self.contentLabel.size = CGSizeMake(self.size.width  - 3*PADDING, size.height);
    self.contentLabel.left = self.avatorButton.left;
...
- (void)layoutSubviews {
    ...
    if(self.images.count>0) {
        UIImageView *iv = [self.imageViews objectAtIndex:self.images.count-1];
